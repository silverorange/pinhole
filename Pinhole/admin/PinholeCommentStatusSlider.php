...
/**
 * Slider widget to select between different comment statuses for Pinhole photos
 *
 ...
 * {@link PinholeCommentStatusSlider::addContextNote()} method.
 *
 * @package   Pinhole
 * @copyright 2009 silverorange
 * @license   http://www.gnu.org/copyleft/lesser.html LGPL License 2.1
 */
class PinholeCommentStatusSlider extends SwatOptionControl
{
	// {{{ public properties
	...
	public function __construct($id = null)
	{
		...
		$this->addJavaScript(
			'packages/pinhole/admin/javascript/pinhole-comment-status-slider.js',
			Pinhole::PACKAGE_ID);

		$this->addStyleSheet(
			'packages/pinhole/admin/styles/pinhole-comment-status-slider.css',
			Pinhole::PACKAGE_ID);
	}
	...
	public function display()
	{
		...
		$container_div->class = 'pinhole-comment-status-slider';
		$container_div->open();

		$thumb_div = new SwatHtmlTag('div');
		$thumb_div->id = $this->id.'_thumb';
		$thumb_div->class = 'pinhole-comment-status-slider-thumb';
		$thumb_div->open();

		$img_tag = new SwatHtmlTag('img');
		$img_tag->class = 'pinhole-comment-status-slider-image';
		$img_tag->src =
			'packages/pinhole/admin/images/pinhole-comment-status-slider-thumb.gif';
	...
	protected function getInlineJavaScript()
	{
		...
		return sprintf(
			"var %s_obj = new PinholeCommentStatusSlider('%s', [%s]);",
			$this->id,
			$this->id,
			$options);
	}
