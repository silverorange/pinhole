/*
 * Returns tree information from the tag tree.
 ...
 *
 * @returned_row type_tag_tree: Contains levelnum, children, id, title, parent and displayorder.
 *
 * Returns a set of returned_rows.
 */
CREATE TYPE type_tag_tree AS (levelnum INTEGER, children INTEGER, id INTEGER, title VARCHAR(255), parent INTEGER, displayorder INTEGER);

CREATE OR REPLACE FUNCTION getPinholeTagTree (INTEGER) RETURNS SETOF type_tag_tree AS $$
	DECLARE
		...
		returned_row type_tag_tree%ROWTYPE;
	BEGIN
		local_level := 0;
		...
		FROM out
		INNER JOIN PinholeTag ON id = item
		ORDER BY out.displayorder, PinholeTag.title DESC
		LOOP
