function PinholeCommentStatusSlider(id, options)
{
	this.id = id;
...
PinholeCommentStatusSlider.prototype.init = function()
{
...
PinholeCommentStatusSlider.prototype.createLabels = function()
{
	...
	for (var i = 0; i < this.options.length; i++) {
		...
		YAHOO.util.Dom.addClass(span, 'pinhole-comment-status-slider-legend');
...
PinholeCommentStatusSlider.prototype.createContextNote = function()
{
	...
	YAHOO.util.Dom.addClass(this.context_note,
		'pinhole-comment-status-slider-context-note');
...
PinholeCommentStatusSlider.prototype.handleChange = function()
{
...
PinholeCommentStatusSlider.prototype.getIndex = function()
{
	return Math.floor(this.slider.getValue() / this.increment);
}

PinholeCommentStatusSlider.prototype.updateContextNote = function()
{
	var index = this.getIndex();
...
PinholeCommentStatusSlider.prototype.updateLegendLabels = function()
{
	var index = this.getIndex();
	for (var i = 0; i < this.options.length; i++) {
		if (i == index) {
			YAHOO.util.Dom.addClass(this.legend_labels[i],
				'pinhole-comment-status-slider-legend-selected');
		} else {
