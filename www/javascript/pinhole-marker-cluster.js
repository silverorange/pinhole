 ...
 *
 * An optional callback method for single-click events has been added by
 * Nick Burka to the original code.
 */
...
MarkerClusterer.double_click = 0;
MarkerClusterer.double_click_timeout = 300;
MarkerClusterer.single_click_callback = function() {};
...
function Cluster(markerClusterer) {
  ...
  this.redraw_ = function (isForce) {
    ...
    if (zoom_ >= mz || this.getTotalMarkers() === 1) {
      ...
      if (clusterMarker_ === null) {
        clusterMarker_ = new ClusterMarker_(markers_, center_, this.getTotalMarkers(), markerClusterer_.getStyles_(), markerClusterer_.getGridSize_());
 ...
 */
function ClusterMarker_(markers, latlng, count, styles, padding) {
  var index = 0;
  var dv = count;
  while (dv !== 0) {
...
ClusterMarker_.prototype.initialize = function (map) {
  ...
  GEvent.addDomListener(div, "dblclick", function () {
    MarkerClusterer.double_click = 2;
  ...
  GEvent.addDomListener(div, "click", function () {
	setTimeout(function() {
      if (MarkerClusterer.double_click > 0) {
        MarkerClusterer.double_click--;
        return;
      }

      var photos = [];
      for (var i = 0; i < self.markers_.length; i++) {
         for (var j = 0; j < self.markers_[i].marker.photos.length; j++) {
            photos.push(self.markers_[i].marker.photos[j]);
		}
      }

      MarkerClusterer.single_click_callback(latlng, photos);

	}, MarkerClusterer.double_click_timeout);
  });
...
ClusterMarker_.prototype.copy = function () {
  return new ClusterMarker_(this.markers_, this.latlng_, this.index_, this.text_, this.styles_, this.padding_);
};
